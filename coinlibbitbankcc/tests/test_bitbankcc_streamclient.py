...
def test_ticker(stream_client: StreamClient):
    ...
    stream_client.subscribe(ticker='XRP_JPY')
    d: StreamData = q.get(timeout=WAIT)
    assert d.key == ('ticker', 'XRP_JPY')
...
def test_order_book(stream_client: StreamClient):
    ...
    stream_client.on_data = q.put
    assert stream_client.wait_connection(5)
    stream_client.subscribe(order_book='XRP_JPY')
    d: StreamData = q.get(timeout=5)
    assert d.key == ('order_book', 'XRP_JPY')
    assert isinstance(d.data, OrderBook)
    assert d.data.instrument == 'XRP_JPY'
    ask = d.data.asks[0]
...
def test_subscribe_unsubscribe(stream_client: StreamClient):
    ...
    d: StreamData = q.get(timeout=WAIT)
    k, d = d.key, d.data
    assert k == ('order_book', 'XRP_JPY')
    assert isinstance(d, OrderBook)
    assert d.instrument == 'XRP_JPY'

    stream_client.unsubscribe(order_book='XRP_JPY')
    ...
    for _ in range(q.qsize() + N):
        ...
        k = d.key
        keys.add(k)
        assert k in [('order_book', 'BTC_JPY'), ('order_book', 'XRP_JPY')]
    assert keys == {('order_book', 'BTC_JPY'), ('order_book', 'XRP_JPY')}
